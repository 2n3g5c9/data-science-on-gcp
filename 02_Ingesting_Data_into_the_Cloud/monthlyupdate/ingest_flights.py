...
def download(year, month, dest_dir):
    ...
    filename = os.path.join(dest_dir, f'{year}_{month}.zip')
...
def zip_to_csv(filename, dest_dir):
    ...
    try:
        ...
        print(f'The file "{filename}" is incorrect.')

    logging.info(f'Extracted "{csv_file}".')
    ...
    try:
        with open(csv_file, 'r') as csv_fp:
            ...
            if first_line != expected_header:
                os.remove(csv_file)
                msg = f'Got header={first_line}, but expected={expected_header}.'
                logging.error(msg)
        ...
        print(f'The file "{csv_file}" has not been found.')
...
def remove_quotes_comma(csv_file, year, month):
    ...
    """
    try:
        out_file = os.path.join(os.path.dirname(csv_file), f'{year}_{month}.csv')
        ...
        logging.debug(f'Ingested "{out_file}" ...')

        return out_file
    finally:
        logging.debug(f'... removing "{csv_file}".')
        os.remove(csv_file)
...
def upload(csv_file, bucket_name, blob_name):
    ...
    logging.info(f'Uploaded "{gcs_location}" ...')
...
def ingest(year, month, bucket):
    ...
    try:
        ...
        gcs_loc = f'flights/raw/{os.path.basename(csv_file)}'
        return upload(csv_file, bucket, gcs_loc)
    finally:
        logging.debug(f'Cleaning up by removing "{temp_dir}".')
...
def next_month(bucket_name):
    ...
    logging.debug(f'The latest file on GCS is "{last_file}".')
    year = last_file[:4]
    month = last_file[5:7]
...
if __name__ == '__main__':
    ...
    try:
        ...
        logging.info(f'Success ... ingested to "{gcs_file}".')
    except DataUnavailable as e:
        logging.info(f'Try again later: {e.message}.')
